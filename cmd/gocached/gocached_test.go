...
func TestExchangeToken(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if d.Scope != tc.wantScopes {
				t.Fatalf("unexpected scope: want %q, got %q", tc.wantScopes, d.Scope)
			}
			...
			if strings.Contains(d.Scope, "write") {
			...
			if stats.Puts == 0 && strings.Contains(d.Scope, "write") {
