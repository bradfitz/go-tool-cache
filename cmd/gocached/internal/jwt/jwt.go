...
// Validate returns an error if the provided JWT fails validation for any reason
// including invalid signature, expiry, incorrect issuer/audience, or
// missing/incorrect claims.
func (v *Validator) Validate(ctx context.Context, jwtString string, wantClaims map[string]string) error {
	...
	if !ok {
		return fmt.Errorf("unexpected claims type: %T", tk.Claims)
	}
	var errs []error
	for k, want := range wantClaims {
		if got, ok := gotClaims[k]; !ok || got != want {
			errs = append(errs, fmt.Errorf("invalid claim %q: expected %q, got %q", k, want, gotClaims[k]))
		}
	}
